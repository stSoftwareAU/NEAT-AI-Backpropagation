...
LIB_FILE="libneat_ai_backpropagation.so"
if [[ "$(uname -s)" == "Darwin" ]]; then
  LIB_FILE="libneat_ai_backpropagation.dylib"
fi
...
set -euo pipefail
if [[ "${1:-}" == "metadata" ]]; then
  printf '%s\n' '{"packages":[{"name":"neat_ai_backpropagation","version":"9.9.9"}]}'
  exit 0
fi
...
printf 'fake\n' >"${HOME}/.cargo/bin/neat_ai_backpropagation"
chmod +x "${HOME}/.cargo/bin/neat_ai_backpropagation"
printf 'fake\n' >"${HOME}/.cargo/lib/${LIB_FILE}"
printf '9.9.9\n' >"${HOME}/.cargo/bin/.neat_ai_backpropagation.version"
printf '9.9.9\n' >"${HOME}/.cargo/lib/.neat_ai_backpropagation.version"
...
assert_eq "already-installed stdout is the CLI path" \
  "${HOME}/.cargo/bin/neat_ai_backpropagation" "${OUT}"
assert_eq "already-installed names the version on stderr" "0" \
  "$(grep -q '\[neat_ai_backpropagation\] already installed v9.9.9' "${WORK_DIR}/already.err"; echo $?)"

echo ""
echo "=== missing stamp is not treated as already installed ==="
rm -f "${HOME}/.cargo/bin/.neat_ai_backpropagation.version"
